...
int main(int argc, char** argv) {
    ...
    printf("[*] DLL: %s\n", dllPath);
    printf("[*] Waiting for game process (VMP.exe / FiveM.exe)...\n");

    const wchar_t* names[] = { L"VMP.exe", L"FiveM.exe", L"FiveM_GTAProcess.exe", L"GTA5.exe" };
    DWORD pid = 0;
    while (!pid) {
        for (auto& n : names) { pid = FindProcessId(n); if (pid) break; }
        if (!pid) Sleep(1000);
    }
    printf("[+] Found game PID=%lu\n", pid);

    printf("[*] Waiting for gta-core-five.dll...\n");
    while (!ModuleExists(pid, L"gta-core-five.dll")) Sleep(500);
    printf("[+] gta-core-five.dll loaded!\n");

    bool ok = InjectDLL(pid, dllPath);
    printf(ok ? "[+] SUCCESS! Press INSERT in game.\n" : "[!] FAILED.\n");
    system("pause");
    return ok ? 0 : 1;
}